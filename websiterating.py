...
def main():
    ...
    parser.add_argument('-P', '--prepareall', action='store_true', default=False, dest='prepareall', help='inital, prepare pages, prepare dataset')
    ...
    parser.add_argument('-A', '--all', action='store_true', default=False, dest='all', help='do all')
    ...
    if args.init or args.prepareall or args.all:
        logger.info("Initail Environment")             
        dm.init_environment()

    if args.pages or args.prepareall or args.all:
        logger.info("preparing websites pages by url")
        dm.prepare_websites_pages_by_url_files()

    if args.dataset or args.prepareall or args.all:
    ...
    if args.train.upper() == 'SGD' or args.trainall or args.all:
    ...
    if args.train.upper() == 'BNB' or args.trainall or args.all:
        logger.info('training with BNB')
        wrm.BernoulliNB_train_model()

    if args.train.upper() == 'SVC' or args.trainall or args.all:
        logger.info('training with SVC')
        wrm.SVC_train_model()

    if args.train.upper() == 'DTC' or args.trainall or args.all:
